        ...
        <Card.Body>
            <div className="d-flex justify-content-between align-items-start">
                <div>
                    ...
                    {subtitulo && <p className="text-muted mb-0 mt-1 small dashboard-kpi__subtitulo">{subtitulo}</p>}
                </div>
                <div className="dashboard-kpi__icone" style={{ color: cor, backgroundColor: `${cor}12` }}>{icone}</div>
            </div>
        </Card.Body>
    </Card>
);
        ...
        if (isGerente || isRecepcionista) {
            promises.push(
                axios.get(`${API_BASE_URL}/financeiro/lancamentos`)
                    .then(r => {
                        ...
                        const receita = lancamentos.filter(l => l.tipo === 'RECEITA' && l.status === 'PAGO').reduce((sum, l) => sum + (parseFloat(l.valor) || 0), 0);
                        const despesa = lancamentos.filter(l => l.tipo === 'DESPESA' && l.status === 'PAGO').reduce((sum, l) => sum + (parseFloat(l.valor) || 0), 0);
                        setStats(prev => ({ ...prev, receita, despesa, saldo: receita - despesa }));
                    })
                    .catch(() => {})
            );
            promises.push(
            ...
            promises.push(
                axios.get(`${API_BASE_URL}/consultas`)
                    .then(r => setStats(prev => ({ ...prev, totalConsultas: r.data.length })))
                    .catch(() => {})
            );
        }
        ...
        <Container className="dashboard-container py-4 px-3 px-md-4">
            ...
            {(isGerente || isRecepcionista) && (
                <Row className="g-3 mb-4">
                    <Col sm={6} xl={3}>
                        <KPI_CARD titulo="Total de Pacientes" valor={stats?.totalPacientes || 0} icone={<IconUser />} cor="#6366F1" />
                    </Col>
                    <Col sm={6} xl={3}>
                        <KPI_CARD titulo="Consultas Hoje" valor={consultasHoje.length || 0}
                            subtitulo={stats?.totalConsultas ? `de ${stats.totalConsultas} no total` : ''}
                            icone={<IconCalendar />} cor="#0EA5E9" />
                    </Col>
                    <Col sm={6} xl={3}>
                        <KPI_CARD titulo="Receita (Pago)" valor={formatarMoeda(stats?.receita || 0)} icone={<IconReceita />} cor="#10B981" />
                    </Col>
                    <Col sm={6} xl={3}>
                        <KPI_CARD titulo="Saldo Financeiro" valor={formatarMoeda(stats?.saldo || 0)}
                            icone={<IconSaldo />} cor={stats?.saldo >= 0 ? '#10B981' : '#EF4444'} />
                    </Col>
                </Row>
            )}
            ...
            {(isDentista && !isGerente && !isRecepcionista) && (
                <Row className="g-3 mb-4">
                    <Col sm={6} xl={3}>
                        <KPI_CARD titulo="Consultas Hoje" valor={consultasHoje.length || 0} icone={<IconCalendar />} cor="#0EA5E9" />
                    </Col>
                </Row>
            )}
                ...
                <Row className="mb-4">
                    <Col>
                        <Card className="shadow-sm">
                            <Card.Header className="bg-white d-flex justify-content-between align-items-center" style={{ borderRadius: '20px 20px 0 0' }}>
                                <div className="d-flex align-items-center gap-2">
                                    <div style={{
                                        width: 32, height: 32, borderRadius: 10,
                                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                                        backgroundColor: '#0f766e15', color: '#0f766e'
                                    }}>
                                        <IconClock />
                                    </div>
                                ...
                                {(isRecepcionista && !isDentista && !isGerente) && (
                                    <Link to="/agendamento" className="btn btn-sm btn-outline-primary rounded-pill">Novo Agendamento</Link>
                                )}
                                {(isDentista || isGerente) && (
                                    <Link to="/consultas/hoje" className="btn btn-sm btn-outline-primary rounded-pill">Ver todos</Link>
                                )}
                            </Card.Header>
                                    ...
                                    <Table className="mb-0" hover responsive>
                                        ...
                                        <tbody>
                                            {consultasHoje.slice(0, 5).map(c => (
                                                <tr key={c.idConsulta} className="dashboard-table-row">
                                                    ...
                                                    <td className="pe-3">
                                                        <Badge bg={statusCor(c.status)} pill>{statusLabel(c.status)}</Badge>
                                                    </td>
                                                </tr>
                                            ))}
                                        </tbody>
            ...
            <Row className="g-3">
                {(isRecepcionista || isGerente) && (
                    <Col sm={6} md={4} xl={3}>
                        <DashboardCard to="/agendamento" icon={<IconCalendar />} title="Agendar Consulta" description="Marque novas consultas e gerencie a agenda" color="#6366F1" />
                    </Col>
                )}
                {(isRecepcionista || isGerente) && (
                    <Col sm={6} md={4} xl={3}>
                        <DashboardCard to="/pacientes" icon={<IconUser />} title="Pacientes" description="Cadastre e gerencie os pacientes" color="#0EA5E9" />
                    </Col>
                )}
                {(isDentista || isGerente) && (
                    <Col sm={6} md={4} xl={3}>
                        <DashboardCard to="/consultas" icon={<IconCheck />} title="Minhas Consultas" description="Visualize e registre atendimentos" color="#10B981" />
                    </Col>
                )}
                {isGerente && (
                    <Col sm={6} md={4} xl={3}>
                        <DashboardCard to="/dentistas" icon={<IconStethoscope />} title="Dentistas" description="Gerencie o corpo clinico" color="#F59E0B" />
                    </Col>
                )}
                {isGerente && (
                    <Col sm={6} md={4} xl={3}>
                        <DashboardCard to="/funcionarios" icon={<IconBuilding />} title="Funcionarios" description="Gerencie a equipe administrativa" color="#EC4899" />
                    </Col>
                )}
                {isGerente && (
                    <Col sm={6} md={4} xl={3}>
                        <DashboardCard to="/procedimentos" icon={<IconWrench />} title="Procedimentos" description="Catalogo de tratamentos e valores" color="#8B5CF6" />
                    </Col>
                )}
                {(isGerente || isRecepcionista) && (
                    <Col sm={6} md={4} xl={3}>
                        <DashboardCard to="/financeiro" icon={<IconCurrency />} title="Financeiro" description="Controle de pagamentos e fluxo de caixa" color="#EF4444" />
                    </Col>
                )}
            </Row>
        </Container>
...
function DashboardCard({ to, icon, title, description, color }) {
    return (
        <Link to={to} className="text-decoration-none" style={{ height: '100%' }}>
            <Card className="h-100 shadow-sm dashboard-link-card" style={{ borderLeft: `3px solid ${color}` }}>
                <Card.Body className="d-flex align-items-center gap-3">
                    <div className="dashboard-link-icon" style={{ color, backgroundColor: `${color}12` }}>{icon}</div>
                    <div>
...
// SVG Icons
function IconUser() {
    return <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="7" r="4"/><path d="M5.5 21a6.5 6.5 0 0113 0"/></svg>;
}
...
function IconClock() {
    return <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>;
}
function IconCheck() {
...
function IconCalendarEmpty() {
    return <svg width="48" height="48" fill="none" stroke="currentColor" strokeWidth="1" viewBox="0 0 24 24" className="opacity-30"><rect x="3" y="4" width="18" height="17" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/></svg>;
}
